...
criteria_stereo = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.01)
...
for i in range(500,
               1500):  # Put the amount of pictures you have taken for the calibration inbetween range(0,?) wenn starting from the image number 0
    t = str(i)
    ChessImaR = cv2.imread('./imgR/imgR_' + t + '.jpg', 0)  # Right side
    ChessImaL = cv2.imread('./imgL/imgL_' + t + '.jpg', 0)  # Left side
